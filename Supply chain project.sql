...
SELECT COUNT(*) FROM chain_supply.product;
SELECT COUNT(*) FROM chain_supply.revenue;
SELECT COUNT(*) FROM chain_supply.shipp;
SELECT COUNT(*) FROM chain_supply.manfucturing;

SELECT COUNT(*) 
FROM chain_supply.shipp
WHERE SKU IS NULL;

ALTER TABLE chain_supply.product
CHANGE COLUMN `ï»¿SKU` SKU VARCHAR(50);

ALTER TABLE chain_supply.shipp
CHANGE COLUMN `ï»¿SKU` SKU VARCHAR(50);

ALTER TABLE chain_supply.revenue
CHANGE COLUMN `ï»¿SKU` SKU VARCHAR(50);

ALTER TABLE  chain_supply.manfucturing
CHANGE COLUMN `ï»¿SKU` SKU VARCHAR(50);
...
CREATE VIEW chain_supply.vw_product_performance AS
SELECT
    p.`Product type`,
    SUM(r.`Revenue generated`) AS Total_Revenue,
    SUM(r.`Number of products sold`) AS Total_Products_Sold,
    COUNT(p.SKU) AS Total_SKUs,
    AVG(r.`Revenue generated`) AS Average_Revenue_Per_SKU,
    AVG(p.Price) AS Average_Price,
    AVG(p.`Stock levels`) AS Average_Stock_Level
FROM chain_supply.product AS p
INNER JOIN chain_supply.revenue AS r
    ON p.SKU = r.SKU
GROUP BY p.`Product type`;

CREATE VIEW chain_supply.vw_executive_summary AS
SELECT
    (
    ...
    (
        ...
        FROM chain_supply.product
    ) AS Average_Stock_Level,

    (
        SELECT AVG(`Manufacturing costs`)
        FROM chain_supply.manfucturing
    ) AS Average_Manufacturing_Cost,
...
SELECT *
FROM chain_supply.vw_executive_summary;

CREATE VIEW chain_supply.vw_inventory_analysis AS
SELECT
    p.SKU,
    p.`Product type`,
    p.`Stock levels`,
    ...
    CASE
        WHEN p.`Stock levels` < 30 THEN 'Low Stock'
        WHEN p.`Stock levels` BETWEEN 30 AND 70 THEN 'Balanced'
        ELSE 'High Stock'
    END AS Inventory_Status
FROM chain_supply.product AS p
INNER JOIN chain_supply.revenue AS r
...
SELECT *
FROM chain_supply.vw_inventory_analysis;

CREATE VIEW chain_supply.vw_supplier_performance AS
SELECT
    m.`Supplier name`,
    SUM(m.`Manufacturing costs`) AS Total_Manufacturing_Cost,
    AVG(m.`Manufacturing costs`) AS Average_Manufacturing_Cost,
    AVG(m.`Defect rates`) AS Average_Defect_Rate,
    AVG(m.`Lead time`) AS Average_Lead_Time,
    SUM(m.`Production volumes`) AS Total_Production_Volume,
    COUNT(m.SKU) AS Total_SKUs
FROM chain_supply.manfucturing AS m
GROUP BY m.`Supplier name`;

SELECT *
FROM chain_supply.vw_supplier_performance;

CREATE VIEW chain_supply.vw_shipping_performance AS
SELECT
    s.`Shipping carriers`,
    AVG(s.`Shipping times`) AS Average_Shipping_Time,
    SUM(s.`Shipping costs`) AS Total_Shipping_Cost,
    AVG(s.`Shipping costs`) AS Average_Shipping_Cost,
    SUM(s.`Order quantities`) AS Total_Order_Quantity,
    COUNT(s.SKU) AS Total_Shipments
FROM chain_supply.shipp AS s
GROUP BY s.`Shipping carriers`;

SELECT *
FROM chain_supply.vw_shipping_performance;